...
def runns(resp_var, size_of_test_data, dataset, positive_class, n_estimators, important_features, dealing_with_nulls):
    ...
    train_features, test_features, train_labels, test_labels = train_test_split(dataset, resp_var,
...
@predictmodelrf.route("/runModel")
def runModel():
    dataset = request.args.get('dataset')
    resp_var = request.args.get('resp_var')
    size_of_test_data = float(request.args.get('size_of_test_data'))
    positive_class = int(request.args.get('positive_class'))
    n_estimators = int(request.args.get('n_estimators'))
    important_features = int(request.args.get('important_features'))
    result = runns(resp_var, size_of_test_data, dataset, positive_class, n_estimators, important_features)
    return result
